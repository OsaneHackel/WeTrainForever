...
import wtf.memory as mem
from wtf.feedforward import Feedforward
#from wtf.util import Memory as mem
#from wtf.util import Feedforward as Feedforward
...
def main():
    ...
    optParser.add_option('-m', '--maxepisodes',action='store',  type='float',
                         dest='max_episodes',default=8000,
    ...
    def save_statistics():
        with open(f"./results/DDPG_{env_name}-eps{eps}-t{train_iter}-l{lr}-s{random_seed}-stat-{run_id}.pkl", 'wb') as f:
    ...
    for i_episode in range(1, max_episodes+1):
        ...
        if i_episode % 500 == 0:
            ...
            path = f'./results/DDPG_{env_name}_{i_episode}-eps{eps}-t{train_iter}-l{lr}-s{random_seed}-{run_id}.pth'
