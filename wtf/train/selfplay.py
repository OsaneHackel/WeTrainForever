...
Modified train script for Self-play of SAC agent. 
Agent pool of weak, strong and frozen states of agent itself can be inserted in 
the agent pool and used for training. 
preload_replay_buffer fills the replay buffer with data from the tournment dataset. 
...
from wtf.utils import create_state_dict, load_agent, load_weights
from wtf.eval.eval import evaluate
...
def preload_replay_buffer(agent, dataset_path, device):
    ...
    n = len(states)
    print(f"Preloading {n} transitions")
...
if __name__ == "__main__":
    ...
    preload_replay_buffer(
        agent,
        dataset_path="./wtf/tournament_dataset.pkl",
        device=device,
    )
